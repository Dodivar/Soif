<template>
  <v-container class="fill-height pa-0" fluid>
    ...
    <div class="position-fixed top-0 right-0 text-h4 text-white my-3">
        <v-icon
        v-if="!wantToCreateRoom && !wantToJoinRoom && !wantToDeletePlayer && !state.room?.roomId"
        @click="goToDeletePlayer()"
        >mdi mdi-cogs</v-icon
      >
      <v-icon
        v-if="!wantToCreateRoom && !wantToJoinRoom && !wantToSetProfil && !state.room?.roomId"
    ...
    <div class="w-100">
      ...
      <div v-else-if="!state.room.roomId && !wantToSelectChampion" class="mx-5 text-white">
        ...
        <v-form class="mt-5 text-center" @submit.prevent>
          ...
          <v-btn
            ...
            elevation="4"
            :loading="state.player.tryToJoinRoom"
            :class="
      ...
      >
        ...
        <div class="d-flex flex-wrap flex-align-center justify-space-around">
          ...
          >
            ...
            <v-btn class="bg-gradient-danger" @click="deletePlayer(player.socketId)">
              Supprimer
            </v-btn>
          </v-badge>
        </div>
      ...
      <div v-else>
        ...
        <!-- ROUND RESULT -->
        <ScoreSoif v-else-if="state.player?.hasPlayed" @confetti="createConfetti" @room-quit="quitRoom" />
        <!-- GAME -->
        <component v-else :is="actualGameName" :class="{ isTazed: state.player.hasBeenTazed > 0 }" @confetti="createConfetti" />
      </div>
...
export default {
  ...
  methods: {
    ...
    joinRoom() {
      state.player.tryToJoinRoom = true
      socket.emit('Room:Join', this.roomToJoin, state.player, this.avatar)

      setTimeout(() => {
        state.player.tryToJoinRoom = false
      }, 10000);
    },
    quitRoom() {
    ...
    goToDeletePlayer() {
      this.notReadyToPlay()
      this.wantToDeletePlayer = !this.wantToDeletePlayer
    },
