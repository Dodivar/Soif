...
export default {
  ...
  methods: {
    createDot() {
      const baseRadius = 20;
      const x = Math.random() * (canvas.width - baseRadius * 2) + baseRadius;
      const y = Math.random() * (canvas.height - baseRadius * 2) + baseRadius;
      ...
      if (colorRandom < 0.65) {
        ...
        color = 'orange';
        lifespan = gameTime;
        growthRate = baseRadius / gameTime; // Growth rate to double size over game duration
      } else {
        color = 'green';
        lifespan = 1000;
        growthRate = 0;
      ...
      this.dots.push({ x, y, baseRadius, radius: baseRadius, color, creationTime, lifespan, growthRate });
    },
    ...
    updateTimer() {
      ...
      this.timerBarWidth = `${(remaining / gameTime) * 100}%`;
      return remaining > 0;
    },
    ...
    gameLoop() {
      ...
      if (Math.random() < 0.05) { // Adjust this value to control dot creation frequency
        this.createDot();
      }
