    ...
    <div class="w-100">
      ...
      <div v-else-if="!allIsReady" class="text-center mx-5">
        ...
        <v-btn
          ...
          @click="wantToConfigureRoom = true"
          elevation="4"
          class="w-100 bg-gradient-info text-white rounded-xl"
	  ...
	  <RoomConfiguration v-else-if="wantToConfigureRoom" />
      <!-- GAMES -->
      <div v-else>
        <!-- GAME DESC -->
        <div v-if="state.room.showNextGamDesc" class="w-100 d-flex justify-center">
          <v-sheet class="w-100 pa-5 text-center rounded-lg mx-5" elevation="8">
            <h2
              class="mb-2"
              :style="state.player.hasBlurRoundDescription &&  state.room.actualGame.canBeBlured ? 'filter: blur(20px);' : ''"
            >
              {{ state.room.actualGame.description }}
            </h2>
			<p><i>Astuce : {{ state.room.actualGame.tips }}</i></p>
