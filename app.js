...
io.on("connection", socket => {
    ...
    socket.on("Room:Create", (player, avatar, roundNumber) => {
        ...
        io.to(roomId).emit('Room:Join', roomObj, newRoomAvatars);
    });
    ...
    socket.on("Room:Join", (roomId, player, avatar) => {
        try {
            ...
            if (!room) {           
                socket.emit('User:JoinRoomKO', `Aucun partie ${roomId} n'a été trouvée`)
                return
            }
            ...
            io.to(roomId).emit('Room:Join', room, actualRoomAvatars);
        }
        catch(e) {
            console.error(e)
            socket.emit('User:JoinRoomKO', `Impossible de rejoindre la partie ${roomId}`)
        }
    });
    ...
    socket.on("Room:Quit", () => {
        ...
        socket.data = {}
        socket.to(roomId).emit("Room:RefreshPlayers", getPlayerRoomState(roomId))
    });
    
    socket.on("Room:DeletePlayer", (socketId) => {
        const roomId = socket.data.actualRoomId

        // socket.leave(roomId) ??
        deletePlayer(roomId, socketId)
        io.to(roomId).emit("Room:RefreshPlayers", getPlayerRoomState(roomId))
        io.to(socketId).emit("User:DeletedFromRoom")
    });
    ...
    socket.on("Game:GiveSoif", (socketId, soifNumber = 1) => {
        ...
        player.reduceFoifToGive(soifNumber)
        io.to(socket.data.actualRoomId).emit("Room:RefreshPlayers", getPlayerRoomState(socket.data.actualRoomId))
    });
    ...
    // WizWaz
    socket.on("WizWaz:SetLooser", (socketId) => {
        const room = getRoom(socket.data.actualRoomId)
    ...
    socket.on("Champion:UsePower", ({championId, targetSocketId}) => {
        ...
        switch (championId) {
            case 'schlag':
                ...
                msg = `${player.pseudo} a reçu ${soif} soif, quel schlag !`
            break
            case 'siphon':
                let total = 0
                const players = getPlayerRoomState(socket.data.actualRoomId)
                players.forEach(p => {
                    if (p.givedSoif || socket.id === p.socketId) return
                    total += p.soifToGive
                    player.addSoifToDrink(p.soifToGive, p)
                    p.reduceFoifToGive(p.soifToGive)
                })
                if (total === 0) return
                msg = `${player.pseudo} a siphonné ${total} soif !`
            break
            case 'victime':
            ...
            case 'taze':
                ...
                msg = `${player.pseudo} a tazé ${targetPlayer.pseudo}, il le sera pendant ${targetPlayer.hasBeenTazed - 1} round`
            break
            default:
            break
        }
        sendMessageToRoom(socket.data.actualRoomId, msg, player)
        io.emit("Room:RefreshPlayers", getPlayerRoomState(socket.data.actualRoomId))
    })
})

function sendMessageToRoom(roomId, msg, player) {
    io.to(roomId).emit("Room:SendMessage", msg, player)
}
...
function playGame(socket, data) {
    ...
    player.gameValueLabel = setGameValueLabel(player, room.actualGame.name)
    ...
    try {
        ...
        switch(room.actualGame.name) {
            ...
            case "ReactionClick":
                room.roundAnswer = Math.min.apply(Math, room.players.filter(e).map(e => e.gameValue));
                break
            case "Labyrinth":
            case "FindEmoji":
