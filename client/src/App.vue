...
#background-app {
  ...
  @for $i from 1 through $total {
    $scale: random(1) - 0.1;
    // .svg
    .bg-emoji:nth-child(#{$i}) {
      left: random(120) * 1% - 20;
      animation: raise#{$i} 1 + random(15) + s linear infinite;
      animation-delay: random(10) - 5 + s;
      transform: scale(1 * $i - 0.1) rotate(random(360) + deg);
      z-index: $i;
      filter: blur($i - 4 + px);
      ...
      @keyframes raise#{$i} {
        to {
          bottom: 0vh;
          transform: scale(1 * $i - 0.6) rotate(random(360) + deg);
        }
      }
    }
  }
