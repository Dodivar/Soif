...
export default {
  ...
  methods: {
    playerHandleClick(target) {
      ...
      if (state.championPowerTarget === true) {
        if (state.player.socketId === target.socketId) {
          ...
          const canBeUSed = this.checkPowerCanBeUsedOnThisPlayer(target.socketId)
          if (canBeUsed === true) {
    ...
    usePlayerChampionPower() {
      if (this.playerChampion.canTarget && state.championPowerTarget === null) {
    ...
    GetChampionName,
    /**
     * Returns true if the champion power can be used on the targeted player.
     * Else return the message why it can't be used
     */
    checkPowerCanBeUsedOnThisPlayer(socketIdTarget) {
      const target = state.players.find(e => e.socketId === socketIdTarget)

      switch(this.playerChampion.id) {
        case 'camping':
          if (player.hasCampingPowerFromSocketId) {
            const campeur = state.players.find(e => e.socketId === player.hasCampingPowerFromSocketId)
            return `Le joueur a déjà été campé par ${campeur.pseudo}, choisie quelqu'un d'autre`
          }
          break
      }
      return true;
